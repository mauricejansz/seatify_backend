...
def toggle_saved_restaurant(request, restaurant_id):
    ...
    saved_entry, created = SavedRestaurant.objects.get_or_create(user=user, restaurant=restaurant)
...
def check_saved_status(request, restaurant_id):
    ...
    """
    user = request.user
    is_saved = SavedRestaurant.objects.filter(user=user, restaurant_id=restaurant_id).exists()
    return Response({"saved": is_saved}, status=200)

@api_view(["POST"])
def send_verification_email(request):
    email = request.data.get("email")
    user = get_object_or_404(Us, email=email)
